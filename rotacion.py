...
def escalar(pto,angulo): # recibe los puntos en una lista ls, y el punto que tiene los escalamientos pEs
    return [pto[0]*(math.cos(angulo)) - pto[1]*(math.sin(angulo)),  pto[0]*(math.sin(angulo)) + pto[1]*(math.cos(angulo))]
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_UP:
                    for p in lsptos:
                        f = trans_pantalla(centro,escalar(cart(centro,p),math.pi))
                ...
                if event.key == pygame.K_DOWN:
                    if cont == 3:
                        for p in lsptos:
                            f = trans_pantalla(centro,escalar(cart(centro,p),(math.pi)/2))
                            lsptos[lsptos.index(p)] = f
                        otrocol = [100,87,120]
                        print (lsptos)
